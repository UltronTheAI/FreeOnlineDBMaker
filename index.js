...
function ap (d) {
    ...
    t = t.replaceAll('%25', '%').replaceAll('%20', ' ').replaceAll('%22', '"').replaceAll('%27', '\'')
    var jdd_ = t.split('%&%')
    console.log(jdd_)
...
app.get('/access', (req, res) => {
    if (process.env.HIDDEN == req.url.replace('/access?', '')) {
        nda1['response'] = nda;
        nda = [];
        res.send(nda1);
    }
...
io.on("connection", (socket) => {
    ...
    var d = new Date();
    nda.push(socket.id + ' connected at ' + String(d.getFullYear() + ' | ' + d.getMonth() + ' | ' + d.getDay() + ' | ' + d.getHours() + ' | ' + d.getMinutes() + ' | ' + d.getSeconds()))
    delete d;
            ...
            fs.readFile(__dirname + '/f.json', 'utf8' , (err, data) => {
                if (err) {
                    ...
                    if (String(data).indexOf(data1) == -1) {
                        ...
                        // data[u_n_id] = {"cid": ""}
                        nda.push({"start": [nd_.getFullYear(), nd_.getMonth(), nd_.getDay(), nd_.getHours(), nd_.getMinutes(), nd_.getSeconds()], "uid": u_n_id, "user_name": data1, "by": socket.id});
                        delete nd_;
...
//   console.log(data)
    ...
    socket.on('acd', (data) => {
        if (data.r.replace(process.env.HIDDEN, '') == '') {
        // console.log('yes');
        // if (data.r == process.env.HIDDEN) {
            nda1['response'] = nda;
            nda = [];
            socket.emit('acd', nda1);
        // }
        }
    });
    ...
    socket.on('disconnect', () =>{
        var d = new Date();
        nda.push(socket.id + ' disconnected at ' + String(d.getFullYear() + ' ' + d.getMonth() + ' ' + d.getDay() + ' ' + d.getHours() + ' ' + d.getMinutes() + ' ' + d.getSeconds()))
        delete d;
    });
